...
class IssueToSubagentController extends Controller
{
    public function index()
    {
        ...
        $allIssuedGoods = BookingOrder::select('tbl_booking_orders.*')
            ->where('tbl_booking_orders.receiver_zone_type','Subagent')
            ->where('tbl_booking_orders.destination_warehouse_id',$warehouseId->id)
            ->orderBy('tbl_booking_orders.date','desc')
            ->get();

        return view('admin.issueToSubagent.index')->with(compact('title','allIssuedGoods'));
    }
    ...
    public function view($senderOrderId)
    {
        ...
        return view('admin.issueToSubagent.view')->with(compact('title','bookedOrder','senderInfo','receiverInfo'));
    }

    public function issueToSubagentStatus(Request $request)
    {
        $bookedOrder = BookingOrder::find($request->bookedOrderId);

        if ($bookedOrder->destination_warehouse_goods_receieve_status == 1)
        {
            $bookedOrder->update( [               
                'destination_warehouse_goods_receieve_status' => 0                
            ]);
        }
        else
        {
            $bookedOrder->update( [               
                'destination_warehouse_goods_receieve_status' => 1                
            ]);
        }
    }
}
