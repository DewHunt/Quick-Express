...
@section('content')
    <form class="form-horizontal" action="{{ route('receiverOrder.saveAssignedDeliveryMan') }}" id="formAddEdit" method="POST" enctype="multipart/form-data" name="form">
    	{{ csrf_field() }}
	    ...
	    <div class="card">
	        <div class="custom-card-header">
	            <div class="row">
	                ...
	                <div class="col-md-6 text-right">
	                	<a class="btn btn-outline-info btn-lg" href="{{ route('receiverOrder.view',$receiverOrderId) }}">
		    ...
		    <div class="card-body">
		    	<div class="row">
		    		<div class="col-md-12">
		    			<input class="form-control" type="hidden" name="bookedOrderId" value="{{ $receiverOrderId }}">
		    		</div>
		    	</div>
		        ...
		        <div class="row">
		            <div class="col-md-10">
		                ...
		                <div class="form-group {{ $errors->has('deliveryManId') ? ' has-danger' : '' }}">
		                    <select class="form-control chosen-select deliveryMan" id="deliveryMan" name="deliveryManId">
		                        <option value="">Select Delivery Man</option>
		                        @foreach ($deliveryMen as $deliveryMan)
		                        	@php
		                        		if ($deliveryMan->id == $bookingOrder->delivery_man_id)
		                        		{
		                        			$select = "selected";
		                        		}
		                        		else
    ...
    <script type="text/javascript">
        $(document).on('change', '#deliveryMan', function(){
            ...
            else
            {
                $.ajax({
                    type:'post',
                    url:'{{ route('receiverOrder.showDeliveryManInfo') }}',
