                ...
                <tbody id="">
                	...
                	@foreach ($warehouses as $warehouse)
                		<tr class="row_{{ $warehouse->id }}">
                			...
                			<td>
                                @php
                                    echo \App\Link::status($warehouse->id,$warehouse->status);
                                @endphp
                			</td>
                			<td>
                    			@php
                    				echo \App\Link::action($warehouse->id);
                    			@endphp                				
                			</td>
                		</tr>
                	@endforeach
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#dataTable tbody').on( 'click', 'i.fa-trash', function () {
                ...
                warehouseId = $(this).parent().data('id');
                var tableRow = this;
                swal({   
                    title: "Are you sure?",   
        ...
        function statusChange(warehouseId) {
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
