                ...
                <table id="dataTable" class="table table-bordered table-striped" width="100%" name="areaTable">
                    <thead>
                        <tr style="background: #00c292; text-align: center;">
                            ...
                            <th style="font-weight: bold; vertical-align: middle;" rowspan="2" width="60px">Issue</th>
                    ...
                    <tbody id="">
                    	@php
                    		$sl = 1;
                    	@endphp
                        @foreach ($allIssuedGoods as $issuedGoods)
                    		<tr class="row_{{ $issuedGoods->id }}">
                    			<td>{{ $sl++ }}</td>
                    			<td>{{ date('d-m-Y', strtotime($issuedGoods->date)) }}</td>
                                <td>{{ $issuedGoods->order_no }}</td>

                                <td>{{ $issuedGoods->sender_zone_type }}</td>
                                @php
                                    $senderInfo = DB::table('view_zones')->select('view_zones.*')->where('zone_id',$issuedGoods->sender_zone_id)->first();
                                @endphp
                                <td>{{ $senderInfo->zone_name }}</td>
                                <td>{{ $senderInfo->zone_phone }}</td>
                                <td>{{ $senderInfo->zone_address }}</td>

                                <td>{{ $issuedGoods->receiver_zone_type }}</td>
                                @php
                                    $receiverInfo = DB::table('view_zones')->select('view_zones.*')->where('zone_id',$issuedGoods->receiver_zone_id)->first();
                                @endphp
                                <td>{{ $receiverInfo->zone_name }}</td>
                                <td>{{ $receiverInfo->zone_phone }}</td>
                                <td>{{ $receiverInfo->zone_address }}</td>

                    			<td>{{ $issuedGoods->delivery_charge }}</td>
                    			<td>
                                    <span class="switchery-demo m-b-30" onclick="statusChange({{ $issuedGoods->id }})">
                                        <input type="checkbox" {{ $issuedGoods->destination_warehouse_goods_receieve_status  == 1 ? 'checked' : '' }} class="js-switch" data-color="#00c292" data-switchery="true" style="display: none;">
                                    </span>
                    			</td>
                    			<td>
                                    @php
                                        echo \App\Link::action($issuedGoods->id);
                                    @endphp
                                </td>
                    		</tr>
                    	@endforeach
    ...
    <script>                
        ...
        function statusChange(bookedOrderId) {
            $.ajax({
                ...
                type: "post",
                url: "{{ route('receiveFromWarehouse.goodsReceiveStatus') }}",
