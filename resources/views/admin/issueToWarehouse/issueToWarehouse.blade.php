	        ...
	        <div class="custom-card-header">
	            <div class="row">
	                ...
	                <div class="col-md-6 text-right">
	                	<a class="btn btn-outline-info btn-lg" href="{{ route('issueToWarehouse.view',$senderOrderId) }}">
		                ...
		                <div class="form-group {{ $errors->has('warehouseId') ? ' has-danger' : '' }}">
		                    <select class="form-control chosen-select warehouse" id="warehouse" name="warehouseId">
		                        ...
		                        @foreach ($warehouses as $warehouse)
		                        	@php
		                        		if ($warehouse->id == $bookingOrder->warehouse_id)
		                        		{
		                        			$select = "selected";
		                        		}
		                        		else
    ...
    <script type="text/javascript">
        $(document).on('change', '#warehouse', function(){
            ...
            else
            {
                $.ajax({
                    type:'post',
                    url:'{{ route('issueToWarehouse.showWarehouseInfo') }}',
